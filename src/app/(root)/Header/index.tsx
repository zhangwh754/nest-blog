    ...
    <>
      <header>
        <div className="flex justify-between items-center">
          ...
          <nav className="hidden ml-auto text-base font-medium leading-5 sm:block">
          ...
          {display && (
            <div className="fixed top-0 left-0 z-10 h-full w-full transform bg-gray-200 opacity-95 duration-300 ease-in-out dark:bg-gray-800 translate-x-0">
              <div className="py-10 px-4 text-white flex justify-end">
                <MenuToggleButton display={false} toggleMenuShow={() => setDisplay(false)} />
              </div>
            </div>
          )}
          ...
          <MenuToggleButton display={true} toggleMenuShow={() => setDisplay(true)} />
        </div>
      </header>
    </>
  )
